...
void populateTimeFile(const duration<double> & time_elapsed, const int & dimension, const int n_threads){
	ofstream file;
	file.open("../output/time_files_concurrent/" + to_string(dimension) + "x" + to_string(dimension) + "_" + to_string(n_threads) + ".txt", ios::app);
...
int main(int argn, char ** argv){
	...
	if(argn <= 1){
		...
		throw logic_error("It is necessary to inform the number of threads!");		
	}
	
	Matrix<int> a(stoi(argv[1]), stoi(argv[2])), b(stoi(argv[1]), stoi(argv[2])), c(stoi(argv[1]), stoi(argv[2]));
	...
	populateTimeFile(time_elapsed, c.length(), stoi(argv[2]));
